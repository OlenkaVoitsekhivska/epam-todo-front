...
export const BoardsReducer = createReducer(
  initialBoardsState,
  on(BoardActions.AddBoard, (state,{board})=>[...state,board]),
  on(BoardActions.GetBoardsSuccess, (state: any, {boards}: any)=> boards),
  on(BoardActions.DeleteBoard, (state, {id})=>state.filter(board=>board.id!==id)),
  on(BoardActions.DeleteBoardSuccess, state=>state)
)